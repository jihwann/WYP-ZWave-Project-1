...
namespace ZWaveLib.CommandClasses
{
    public class DoorLock : ICommandClass
    {
        ...
        public static ZWaveMessage Set(ZWaveNode node, Value value)
        {
            ...
            return node.SendDataRequest(new byte[] { // revised by JH for test
                //(byte)CommandClass.DoorLock, 
        ...
        public static ZWaveMessage Set1(ZWaveNode node, Value value) // for test
        {
            byte lockValue1;
            if (value == DoorLock.Value.Secured)
                lockValue1 = 255;
        ...
        public static ZWaveMessage Set2(ZWaveNode node, Value value) // for test
        {
            byte lockValue1;
            if (value == DoorLock.Value.Secured)
                lockValue1 = 255;
        ...
        public static ZWaveMessage Set1(ZWaveNode node, Value value, byte[] request) // for test
        {
            byte lockValue1;
            if (value == DoorLock.Value.Secured)
                lockValue1 = 255;
